...
const Exq_to_oban = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<p data-svelte-h="svelte-ipq8l8">I’m the sole maintainer for a sales portal, processing about a million dollars a day in sales. The system was, historically, backed by Exq and Redis running on AWS. An order processing state machine is
...
the system was built background job processing similar to Ruby’s Sidekiq made sense.</p> <p data-svelte-h="svelte-1r910bu">Since the get-go we experienced the occasional instance where orders would halt en masse. Solutions ranged from emptying the Exq queue, calling a full <code>FLUSHALL</code> in Redis, all the way to restarting the
...
out of the equation.</p> <p data-svelte-h="svelte-eawufa">Oban is such an easy library to work with. Working off a 40hr/month retainer and continuing to troubleshoot other issues and develop new features I managed to move the job processing entirely to
...
timeout we imposed on a specific queue.</p>`;
});
export {
  Exq_to_oban as default,
  metadata
};
